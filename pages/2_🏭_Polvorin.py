...
st.set_page_config(page_title="Polvorín", page_icon="🏭", layout="wide")
require_login()
st.session_state.setdefault("polvorines", [])
st.session_state.setdefault("puntos_riesgo", [])

st.title("🏭 Seguridad de polvorín")
st.write(
...
with st.expander("Zona UTM del proyecto"):
    c1, c2 = st.columns(2)
    with c1:
...
with st.expander("➕ Agregar polvorín", expanded=len(st.session_state["polvorines"]) == 0):
    with st.form("form_polvorin", clear_on_submit=True):
        ...
        enviado_p = st.form_submit_button("Agregar polvorín")
        if enviado_p:
            if not nombre_p:
...
with st.expander("➕ Agregar punto de riesgo", expanded=False):
    with st.form("form_punto_riesgo", clear_on_submit=True):
        ...
        enviado_r = st.form_submit_button("Agregar punto de riesgo")
        if enviado_r:
            if not nombre_r:
...
st.header("Polvorines registrados")
for polvorin in polvorines:
    with st.container(border=True):
        ...
        with c2:
            if st.button("Eliminar", key=f"del_{polvorin.nombre}"):
